...
class CRF:
    ...
    def create_vector_list(self, x_vecs, y_vecs):
        ...
        for i in range(len(y_vecs)):
            ...
            y_vecs[i].append(END)
            labels.append(np.array([self.label_id[y] for y in y_vecs[i]], copy=False, dtype=np.int))
    ...
    def neg_likelihood_and_deriv(self, x_vec_list, y_vec_list, w, debug=False):
        ...
        for x_vec, y_vec in zip(x_vec_list, y_vec_list):
            ...
            # 计算特征函数关于模型的期望,(0,1,2) 表示三个维度均相加
            exp_features = np.sum(np.exp(log_probs) * all_features, axis=(0, 1, 2))
            # 计算特征函数关于训练数据的期望
            emp_features = np.sum(all_features[:, yp_vec_ids, y_vec_ids], axis=0)
            # 计算似然函数
            likelihood += np.sum(log_M_s[:, yp_vec_ids, y_vec_ids]) - log_Z
            # 计算似然函数的偏导
